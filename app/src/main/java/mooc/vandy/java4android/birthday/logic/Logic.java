...
public class Logic
    ...
    public double calculate(int size, int count) {
        ...
        double sameBirthdays = 0;
        for(int i = 0; i < count; i++){
            sameBirthdays += verify(size, i);
        }
        return sameBirthdays/count * 100;
    }


    // TODO -- add your helper methods here
    public int verify(int size, int i){
        List<Integer> numbers = new ArrayList<>();
        for(int j = 0; j < size; j++) {
            Random random = new Random((i * size) + j);
            int randomNumber = random.nextInt(366);
